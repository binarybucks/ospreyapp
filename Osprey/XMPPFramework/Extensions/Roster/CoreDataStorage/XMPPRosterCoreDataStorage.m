...
- (void)beginRosterPopulationForXMPPStream:(XMPPStream *)stream
{
	...
	// In the stock XMPPFramework the roster core data store was cleared and build from the ground up on every start of the application
    // However, we don't do this as we actually want to preserve our changes (such as chatOpened) and cached user images
    // Thus, the following block is commented out
